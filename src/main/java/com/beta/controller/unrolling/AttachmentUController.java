 ...
 */
@Controller
@RequestMapping(value="/attachmentu")
public class AttachmentUController extends BaseController {
	
	String menuUrl = "attachmentu/list.do"; //菜单地址(权限用)
	@Resource(name="attachmentuService")
	private AttachmentUManager attachmentService;
	...
	public ModelAndView save() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"新增AttachmentU");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "add")){return null;} //校验权限
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
//		pd.put("ATTACHMENT_ID", this.get32UUID());	//主键
		pd.put("ATTACHMENT_DATE", Tools.date2Str(new Date()));				//上传时间
		pd.put("ATTACHMENT_USERNAME", Jurisdiction.getUsername());				//上传者
	...
	public void delete(PrintWriter out) throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"删除AttachmentU");
	...
	public ModelAndView edit() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"修改AttachmentU");
	...
	public ModelAndView list(Page page) throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"列表AttachmentU");
		...
		mv.setViewName("beta/attachmentu/attachment_list");
	...
	public ModelAndView goAdd(Page page)throws Exception{
		...
		pd = this.getPageData();
		page.setPd(pd);
		List<PageData> unList = unrollingService.listU(page);
		mv.setViewName("beta/attachmentu/attachment_edit");
	...
	public Object deleteAll() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"批量删除AttachmentU");
	...
	public ModelAndView exportExcel() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"导出AttachmentU到excel");
		...
		for(int i=0;i<varOList.size();i++){
			...
			vpd.put("var2", varOList.get(i).getString("ATTACHMENT_NAME"));	    //2
			vpd.put("var3", varOList.get(i).getString("ATTACHMENT_USERNAME"));	    //3
			vpd.put("var4", varOList.get(i).getString("ATTACHMENT_DESCRIPTION"));	    //4
			vpd.put("var5", varOList.get(i).getString("ATTACHMENT_DATE"));	    //5
			vpd.put("var6", varOList.get(i).getString("ATTACHMENT_SIZE"));	    //6
			vpd.put("var7", varOList.get(i).getString("ATTACHMENT_PATH"));	    //7
			vpd.put("var8", varOList.get(i).get("UNROLLING_ID").toString());	//8
			varList.add(vpd);
		}
